...
- 입출력 클래스와 데코레이터 패턴
  ...
  - RandomAccessFile 클래스 : 입출력 클래스 중 유일하게 파일 입출력을 동시에 할 수 있는 클래스, 파일 포인터가 있어서 읽고 쓰는 위치의 이동이 가능, 다양한 자료형에 대한 메서드가 제공
  - 데코레이터 패턴
- Thread
  - Process : 실행중인 프로그램, OS로부터 메모리를 할당 받음
  - Thread : 실제 프로그램이 수행되는 작업의 최소 단위, 하나의 프로세스는 하나 이상의 Thread를 가지게 됨
  - 구현: 1. Thread 클래스로 상속받아 구현 2. Runnable 인터페이스 구현
  - 멀티쓰레드 프로그래밍 : 공유 자원이 있는 경우, critical section에 대한 동기화의 구현이 필요
  - interrupt
  - 데드락 방지, static 키워드 가지는 변수들에서 발생.